...
def test_all_transitions():
    """9개 트랜지션을 모두 테스트하는 함수"""
    
    print("🚀 9개 트랜지션 전체 테스트 시작...")
    print(f"📝 사용할 영상 개수: {len(SAMPLE_VIDEOS)}")
    
    # VideoTransitionMerger 인스턴스 생성 (static 디렉토리 사용)
    ...
    print(f"\n🎯 총 {len(transitions)}개의 트랜지션을 생성합니다.")
    
    try:
        # 모든 트랜지션을 포함한 쇼케이스 영상 생성
        start_time = time.time()
        output_path = merger.create_sequential_showcase(
            sample_videos=SAMPLE_VIDEOS,
            output_filename="test_9_transitions_showcase.mp4"
        )
        end_time = time.time()
